...
даёт VPS команду забрать образ и перезапуститься.

Портов наружу сервер не публикует. 80 и 443 на машине держит **общий
reverse-proxy** — отдельный стек, который фронтит и другие проекты и сам же
продлевает сертификаты. Мы просто стоим с ним в одной docker-сети `web`, и он
ходит к нам по имени контейнера.

```
push → GitHub Actions → ghcr.io/r0-0ky/g-launcher/server:latest
                              ↓ ssh: docker compose pull && up -d
   игроки → reverse-proxy :80/:443 ──сеть web──→ gandoni-launcher-server:8080
```
...
Нужен сервер с Ubuntu/Debian и SSH-доступом.
...
### 2. Место в reverse-proxy

Сеть должна существовать до первого запуска — её заводит стек прокси:

```bash
docker network create web   # если её ещё нет
```

Дальше в стеке прокси нужен блок под наш домен. У нас это nginx с шаблонами:
в его `.env` добавляются `GLAND_DOMAIN` и `GLAND_UPSTREAM=gandoni-launcher-server:8080`,
те же имена дописываются в `NGINX_ENVSUBST_FILTER`, а рядом с остальными
шаблонами кладётся `gland.conf.template`. Если сеть у тебя называется иначе,
задай `PROXY_NETWORK` в нашем `.env`.

Сертификат выпускается разово, тем же certbot, что стоит у прокси:

```bash
cd /srv/reverse-proxy
docker compose run --rm --entrypoint certbot certbot certonly \
  --webroot -w /var/www/certbot -d launcher.example.com --agree-tos -m почта@пример -n
docker compose up -d proxy
```

Продлевать не надо — цикл `certbot renew` у прокси уже крутится.
...
Переключать надо **до** выпуска сертификата: Let's Encrypt подтверждает владение
через 80-й порт, то есть должен попасть на эту машину.

### 4. Доступ для GitHub Actions
...
- **IP машины виден.** Он больше не спрятан за CDN, так что защиты от мусорных
  запросов тоже нет — как минимум закрой всё, кроме 22, 80 и 443.
- **Размер загрузки ничем не ограничен.** Ни сервером, ни nginx: в блоке нашего
  домена стоит `client_max_body_size 0`, иначе мод срезало бы на дефолтном
  мегабайте.
- **Сертификат продлевается сам** — контейнером `certbot` в стеке прокси, ключи
  лежат в `/etc/letsencrypt` на хосте. Посмотреть, как идут дела:
  `docker compose -f /srv/reverse-proxy/docker-compose.yml logs certbot`.
...
Сами установщики лежат на GitHub, но игрокам отдаются через нас —
`/download/file/<имя>`. Прямые ссылки ведут на `objects.githubusercontent.com`,
а он доступен не отовсюду; сервер тянет файл и переливает потоком, ничего не
сохраняя у себя. Трафик при этом идёт через VPS.
...
# логи прокси и сертификатов — в соседнем стеке
docker compose -f /srv/reverse-proxy/docker-compose.yml logs -f proxy
docker compose -f /srv/reverse-proxy/docker-compose.yml logs -f certbot
...
| `PROXY_NETWORK` | `web` | Внешняя docker-сеть, в которой стоит reverse-proxy |
| `PORT` | `8080` | Порт HTTP |
