...
Тарифы задаются в админке, вкладка **Коины**: сколько G-коинов и за сколько
рублей. Игрок видит их отдельной страницей в лаунчере — она открывается кликом
по кошельку в магазине и сама по себе, если монет не хватило на покупку.

Платит игрок картой на странице Т-банка, в браузере: форму держит банк, в
лаунчер данные карты не попадают. Дальше исход платежа приходит к нам двумя
путями сразу — банк присылает уведомление на `/webhook/tbank`, а лаунчер
параллельно опрашивает статус, и сервер при необходимости спрашивает банк сам.
Начисление одноразовое: сколько бы раз банк ни повторил уведомление, монеты
добавятся один раз.

Чтобы это заработало, в `.env` нужны `TBANK_TERMINAL_KEY` и `TBANK_PASSWORD`, а
в личном кабинете банка — наш адрес уведомлений. Ставить его руками не
обязательно: он передаётся при создании каждого платежа.

Банк выдаёт два терминала — тестовый и боевой, у каждого свои ключ и пароль
(в кабинете: Магазины → магазин → Терминалы). Адрес API у них общий, так что
`TBANK_API_URL` для проверки менять не надо — достаточно подставить ключи
тестового.

Сертификат банка выпущен национальным удостоверяющим центром, и его корня нет
ни в Node, ни в системе: без него соединение обрывается, а игрок видит «банк не
принял платёж». Поэтому корень лежит в [`assets/russian-trusted-root-ca.pem`](assets/)
и добавляется к обычным только для запросов в банк. Делать что-либо с ним
руками — ни локально, ни на сервере — не нужно.
...
| `TBANK_TERMINAL_KEY` | — | Терминал Т-банка. Без него пополнение кошелька скрыто |
| `TBANK_PASSWORD` | — | Пароль терминала: им подписываются запросы и уведомления |
| `TBANK_RETURN_URL` | пусто | Куда банк вернёт игрока после оплаты |
| `TBANK_API_URL` | боевой | Адрес API банка — меняют для тестового контура |
| `PORT` | `8080` | Порт HTTP |
