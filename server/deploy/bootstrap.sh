...
# Домен, на котором стоит сервер. Попадает в ссылки манифеста и скинов.
...
Готово. Осталось:
  ...
  3. Завести сеть reverse-proxy, если её ещё нет: docker network create web
     и прописать домен в его конфиге. A-запись направить на эту машину.
  4. Пушнуть в main — workflow deploy-server зальёт compose и поднимет сервис.
