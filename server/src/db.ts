...
export const queries = {
  ...
  /**
   * Отметить оплаченным. Условие в конце тут не для красоты: это оно делает
   * начисление одноразовым, сколько бы раз банк ни повторил уведомление.
   * Вызывающий смотрит на число изменённых строк.
   *
   * Проверяем именно «ещё не оплачен», а не «ждёт оплаты»: игрок мог сначала
   * ошибиться картой, а потом заплатить другой — по тому же заказу. Требуй мы
   * `pending`, деньги бы списались, а монеты не начислились.
   */
  markPaid: db.prepare(
